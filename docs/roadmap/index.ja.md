...
再デプロイは不要になります。詳細は [Edge / Cloud パイプライン分割](./../pipeline/edge-cloud-split.md) を参照してください。
...
設計詳細は `_inputs/edge_cloud_pipeline.md` を参照。
...
- `_inputs/edge_cloud_pipeline.md` — エッジ / クラウド分割の設計詳細
