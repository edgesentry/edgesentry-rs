...
CLS / ETSI EN 303 645 / JC-STAR 要件とソースコードの詳細な条項別マッピングは、[コンプライアンス・トレーサビリティマトリックス](../security/cls-traceability.ja.md)を参照してください。
...
- シンガポール CLS/iM8 条項とソースコードのトレーサビリティマトリックス：[コンプライアンス・トレーサビリティマトリックス](../security/cls-traceability.ja.md)
...
- [SBOM とベンダー開示](../security/sbom.ja.md)および [#92](https://github.com/edgesentry/edgesentry-rs/issues/92)を参照してください
...
- STRIDE の脅威モデル（対象：なりすまし（デバイス同一性）・改ざん（監査レコード）・否認（操作ログ）・情報漏洩（ペイロードストレージ）・サービス拒否（ネットワークポリシー）・特権昇格（インジェストゲート））— [脅威モデル](../security/threat-model.ja.md) を参照
- 出荷クレートに既知の CVE がないことを確認するバイナリ分析レポート（`cargo-audit`・`cargo-deny`）
- 脅威モデルの緩和策をトレーサビリティマトリックスエントリにリンク— [コンプライアンス・トレーサビリティマトリックス](../security/cls-traceability.ja.md)（「防御の厳格性」を ✅ に更新済み）
- 日本語訳：[`docs/security/threat-model.ja.md`](../security/threat-model.ja.md)
