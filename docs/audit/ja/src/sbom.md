...
| 転送暗号化 | レコードレベル：ペイロードハッシュへの Ed25519 署名。ネイティブ TLS トランスポートを提供：`eds serve-tls --tls-cert / --tls-key`（rustls TLS 1.2/1.3、HTTP）および `eds serve-mqtt --tls-ca-cert`（MQTT over TLS）。[トレーサビリティマトリックスの CLS-05](traceability.md) を参照。 |
