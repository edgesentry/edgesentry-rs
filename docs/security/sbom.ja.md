...
| 転送暗号化 | レコードレベル：ペイロードハッシュへの Ed25519 署名。ネイティブ TLS トランスポートを提供：`eds serve-tls --tls-cert / --tls-key`（rustls TLS 1.2/1.3、HTTP）および `eds serve-mqtt --tls-ca-cert`（MQTT over TLS）。[トレーサビリティマトリックスの CLS-05](cls-traceability.md) を参照。 |
...
| ソフトウェア更新の完全性 | `UpdateVerifier` が更新適用前に BLAKE3 ペイロードハッシュと Ed25519 パブリッシャー署名を確認 — [CLS-03](cls-traceability.md) 参照 |
...
このドキュメントは[ロードマップ](../audit/roadmap.md)のマイルストーン 1.4 を満たします。条項別のコンプライアンスマッピングの詳細は[コンプライアンス・トレーサビリティマトリックス](cls-traceability.md)を参照してください。
