...
構造化された海事航海データを `DocumentEntity` JSONL に解析します。
ファイル形式は拡張子から自動判別されます：

- `.parquet` — maridb が出力する Parquet ファイル（本番/大量データ）
- それ以外 — CSV（フィクスチャ/手動作成データ）
...
| フラグ | 説明 |
|------|-------------|
| `--source` | 入力ファイル：`.parquet`（maridb 出力）または `.csv`（フィクスチャ） |
