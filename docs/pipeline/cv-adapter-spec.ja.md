...
本番アーキテクチャの推奨は、産業環境（港湾ターミナル・倉庫・建設現場）向けに
認定済みの検出モデルを持つ専門 CV ベンダーとの統合です。
...
**ステータス：** フォールバック——現場 PoC 時に CV ベンダーとの統合がまだ整っていない場合のみ使用

specula は最小限の自前 OSS ベース CV adapter です。本番システムでも注力領域でもありません。
ベンダー統合が整う前に現場 PoC が必要になった場合のブロッカー解消が目的です。
