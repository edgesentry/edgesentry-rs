...
エンジニアリング工数は物理エンジン・ルール評価・監査チェーン・証拠インフラに集中させており、
物体検出には注力しません。

PoC サイトが確定した時点で、産業向け AI 画像検査の専門ベンダーである
**MENOU**（[menou.co.jp](https://menou.co.jp/home_en)）に協力を依頼します。
MENOU は製造業向け AI 画像検査で実績のある専門企業であり、
edgesentry-rs の入力層として必要な物体検出・追跡能力を持ちます。
...
**ステータス：** フォールバック——現場 PoC 時に MENOU との協力体制がまだ整っていない場合のみ使用

specula は最小限の自前 OSS ベース CV adapter です。本番システムでも注力領域でもありません。
ベンダーとの協力体制が整う前に現場 PoC が必要になった場合のブロッカー解消が目的です。
エンジニアリング工数は物理エンジンと監査チェーンに集中させています。
