...
- `transport-tls` フィーチャー：rustls TLS 1.2/1.3 による`serve_tls()`；`eds serve --tls-cert / --tls-key` CLI；CLS-05 チャネル機密性を満足 — [#73](https://github.com/edgesentry/edgesentry-rs/issues/73) クローズ
