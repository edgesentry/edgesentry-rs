...
mod tests {
    ...
    fn copy_str_fits() {
        let mut buf = [0u8; 8];
        assert_eq!(copy_str_to_buf("hello", &mut buf), EDS_OK);
        assert_eq!(&buf[..6], b"hello\0");
    }

    #[test]
    fn copy_str_exactly_fits() {
        let mut buf = [0u8; 6]; // "hello" (5) + NUL (1)
        assert_eq!(copy_str_to_buf("hello", &mut buf), EDS_OK);
        assert_eq!(buf[5], 0);
    }

    #[test]
    fn copy_str_too_long() {
        let mut buf = [0u8; 5]; // "hello" needs 6 bytes with NUL
        assert_eq!(copy_str_to_buf("hello", &mut buf), EDS_ERR_STRING_TOO_LONG);
    }

    // ── read_str_from_buf ─────────────────────────────────────────────────────

    #[test]
    fn read_str_stops_at_null() {
        let mut buf = [0u8; 16];
        buf[..5].copy_from_slice(b"hello");
        assert_eq!(read_str_from_buf(&buf), "hello");
    }

    #[test]
    fn read_str_no_null_uses_full_buffer() {
        let buf = [b'x'; 4];
        assert_eq!(read_str_from_buf(&buf), "xxxx");
    }
    ...
    fn sign_record_device_id_too_long_returns_error() {
        let mut rec = zeroed_record();
        let long_id = "x".repeat(256); // 256 chars + NUL would need 257 bytes; buf is 256
    ...
    fn sign_record_object_ref_too_long_returns_error() {
        let mut rec = zeroed_record();
        let long_ref = "x".repeat(512); // 512 chars + NUL would need 513 bytes; buf is 512
        let dev = CString::new("dev").unwrap();
    ...
    fn sign_record_fills_fields_correctly() {
        ...
        assert_eq!(read_str_from_buf(&rec.device_id), "sensor-01");
        assert_eq!(read_str_from_buf(&rec.object_ref), "sensor-01/check-1.bin");
