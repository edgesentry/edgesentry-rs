...
pub fn render_markdown(report: &Report) -> String {
    ...
    if let Some(valid) = report.chain_valid {
        ...
        if valid {
            out.push_str(
                "**PASS** — All recorded events have been verified as unmodified. \
                No tampering, deletion, or reordering has occurred. \
                This log can be presented as evidence in a safety inspection or insurance claim.\n\n"
            );
        } else {
            out.push_str(
                "**FAIL** — Integrity verification failed. \
                One or more records may have been modified after the fact. \
                This log should not be used as evidence without investigation.\n\n"
            );
        }
    }
...
pub fn render_pdf(report: &Report) -> Vec<u8> {
    ...
    if let Some(ref summary) = report.executive_summary {
        line!(font_bold, 14.0_f32, left, y, "Executive Summary");
        y -= 7.0;
        // Word-wrap the summary text at ~90 chars per line
        let words: Vec<&str> = summary.split_whitespace().collect();
        let mut current_line = String::new();
        for word in words {
            if current_line.len() + word.len() + 1 > 90 {
                line!(font, 9.5_f32, left, y, current_line.clone());
                y -= 5.5;
        ...
        if !current_line.is_empty() {
            line!(font, 9.5_f32, left, y, current_line);
            y -= 5.5;
        }
        y -= 5.0;
    }
