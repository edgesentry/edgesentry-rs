...
/// Convert (lat, lon) degrees to port-local (x=east, y=north) in metres.
///
/// Uses equirectangular approximation: fast and accurate within port scale.
...
mod tests {
    ...
    fn latlon_to_local_one_degree_north() {
        ...
        assert!((y - 111_320.0_f32).abs() < 200.0, "y should be ~111320, got {y}");
    }
